...
export default function Register() {
  ...
  const fullnameFromGoogle = location.state?.fullname || "";
  const emailFromGoogle = location.state?.email || "";

  const [fullname] = useState(fullnameFromGoogle);
  const [email] = useState(emailFromGoogle);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!birthdate || !phone || hasAllergies === null || (hasAllergies && selectedAllergies.length === 0)) {
  ...
  try {
    await authAPI.updateUser({
      userId: location.state?.userId, // ต้องส่ง id ของผู้ใช้จาก state หรือ context
      user_phone: phone,
      user_birthdate: birthdate,
      user_allergies: hasAllergies ? selectedAllergies : [],
    });

      alert("กรอกข้อมูลเพิ่มเติมเรียบร้อย");
      navigate("/dashboard"); // ไปหน้า login หลังบันทึก
    } catch (error) {
